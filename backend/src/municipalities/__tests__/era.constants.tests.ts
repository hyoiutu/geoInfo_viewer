...
describe('isMunicipalityEraに関するテスト', () => {
  ...
  test('MUNICIPALITY_ERA_TAISHOは大正時代(1920-01-01)を表し、MUNICIPALITY_ERASに含まれる', () => {
    expect(MUNICIPALITY_ERA_TAISHO).toBe('1920-01-01');
    expect(MUNICIPALITY_ERAS).toContain(MUNICIPALITY_ERA_TAISHO);
  });
});
