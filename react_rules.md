...
このルールが対象とするのは上記6種類のHooks呼び出し同士の順序のみであり、Hooksではない通常の関数定義（イベントハンドラ等）の配置は対象外とする。あるカスタムフック呼び出しが、直前に定義したコールバック関数を引数として要求する場合（例: `useCyclingActivities(visibility, onSyncComplete)`のように、コールバックをフックへ渡す設計）、そのコールバックは（JavaScriptの変数束縛の都合上）該当フック呼び出しの直前に置く必要があり、これは「Hooksグルーピング違反」ではない。同じ役割のコールバックが他のフックでは末尾のハンドラ群にまとめられているとしても、それは「そのフックには不要な引数だった」だけであり、比較対象として矛盾を指摘する根拠にはならない（PR #110レビュー対応。AIレビューがこの違いを取り違えて誤検知した実例あり）。
