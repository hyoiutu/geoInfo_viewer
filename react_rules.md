...
同じ種類のHooks呼び出しが複数ある場合はグループ内でまとめる（間に別の種類を挟まない）。Hooks呼び出し同士に依存関係がある場合（例: Aの戻り値をBの引数に渡す）は、その依存順を優先してよい（PR #69レビュー対応）。この例外はカスタムフック同士の依存に限らず、`useMemo`の結果をカスタムフックの引数として渡す場合など、種類の異なるHooks間の依存にも適用される（例: `useMemo`でフィルタ適用済みの一覧を計算し、その結果をカスタムフックへ渡す場合、`useMemo`が直前に来る配置は許容される。PR #128レビュー対応）。
