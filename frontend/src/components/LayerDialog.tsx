...
type LayerDialogProps = {
   ...
   * 年代選択プルダウンを無効化するのに加え、`AppDialog`の`closeDisabled`propへ渡すことで閉じる(×)ボタン・
   * Escapeキー・背景クリックによるクローズも全て無効化する。実行ボタンの無効化は、待機中の多重実行によって
   * MapWorkspace側のpendingLayerApplyが上書きされ未完了の非同期処理の追跡が失われる不具合を防ぐ
   * （Issue #65 PR#110レビュー対応）。他の入力・クローズ手段の無効化は、待機完了時にダイアログが
   * 自動的に閉じ入力中(draft)の内容が破棄される際、待機中に加えた変更・操作がユーザーの意図しないまま
   * 失われることを防ぐ（PR #110再レビュー対応）
   */
