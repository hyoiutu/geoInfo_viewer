...
describe('MapControlsに関するテスト', () => {
  test('レイヤー・フィルタ・設定の3つのアイコンボタンが表示される', () => {
    renderWithChakra(
      <MapControls onOpenLayerDialog={vi.fn()} onOpenFilterDialog={vi.fn()} onOpenSettingsDialog={vi.fn()} />
    );
  ...
  test('レイヤーアイコンを押すと、onOpenLayerDialogが呼ばれる', () => {
    ...
    renderWithChakra(
      <MapControls onOpenLayerDialog={onOpenLayerDialog} onOpenFilterDialog={vi.fn()} onOpenSettingsDialog={vi.fn()} />
    );
  ...
  test('フィルタアイコンを押すと、onOpenFilterDialogが呼ばれる', () => {
    ...
    renderWithChakra(
      <MapControls onOpenLayerDialog={vi.fn()} onOpenFilterDialog={onOpenFilterDialog} onOpenSettingsDialog={vi.fn()} />
    );
