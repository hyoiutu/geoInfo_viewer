...
describe('useActivitySelectionに関するテスト', () => {
  ...
  test('selectActivitiesを呼ぶと、指定したIDに対応するアクティビティが選択される', () => {
    ...
    const { result } = renderHook(() => useActivitySelection(activities));
  ...
  describe('visibleActivitiesの変化による自動除外に関するテスト（Issue #127）', () => {
    test('visibleActivitiesが変化しても、選択中のアクティビティが全て含まれ続ける場合は選択・フォーカスともに変化しない', () => {
      const activities = [
        createActivity({ id: '1', elevationGainMeters: 100 }),
        createActivity({ id: '2', elevationGainMeters: 200 })
      ];
      const { result, rerender } = renderHook(({ visibleActivities }) => useActivitySelection(visibleActivities), {
        initialProps: { visibleActivities: activities }
      });
      act(() => {
        result.current.selectActivities(['1', '2']);
        result.current.focusActivity(1);
      ...
      rerender({ visibleActivities: activities });

      expect(result.current.selectedActivities).toEqual(activities);
      expect(result.current.focusedActivity).toEqual(activities[1]);
    });

    test('選択中のアクティビティのうちvisibleActivitiesから外れたものを選択から取り除く', () => {
      const activities = [
        createActivity({ id: '1', elevationGainMeters: 100 }),
      ...
      rerender({ visibleActivities: [activities[0], activities[2]] });

      expect(result.current.selectedActivities).toEqual([activities[0], activities[2]]);
    });

    test('フォーカス中のアクティビティがvisibleActivitiesから外れた場合、フォーカスを解除する', () => {
      const activities = [
        createActivity({ id: '1', elevationGainMeters: 100 }),
        createActivity({ id: '2', elevationGainMeters: 30 })
      ];
      ...
      rerender({ visibleActivities: [activities[0]] });

      expect(result.current.selectedActivities).toEqual([activities[0]]);
      expect(result.current.focusedActivity).toBeNull();
    });

    test('フォーカス中のアクティビティがvisibleActivitiesに含まれ続ける場合、選択が縮んでもフォーカスは同じアクティビティを指し続ける', () => {
      const activities = [
      ...
      rerender({ visibleActivities: [activities[1], activities[2]] });
