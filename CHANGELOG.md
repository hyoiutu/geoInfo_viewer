...
* **各ファイルへの影響と変更内容**:
  * **実装**:
    - 新規`PhotoPreviewModal`（共通の`AppDialog`をベースにしたダイアログ）を追加。矢印キー（→/←）での前後移動、先頭・末尾での移動ボタン無効化に対応。
    - `MapWorkspace.tsx`に`previewPhotoIndex`状態と`handlePhotoClick`を追加し、パネル・吹き出し両方のクリックを同じハンドラで受けて同じダイアログを開くようにした（見た目・挙動の共通化）。
    - `PhotoGridItem`（`ActivityDetailSidebar.tsx`）に`onClick`を追加。`PhotoBalloonThumbnail`は、独立したReact root（`ChakraProvider`配下外）にあるため`<button>`でラップしてクリック領域とした（`<img>`は非インタラクティブ要素のためbiomeのa11yルールに従い`<button>`を使用）。
  * **README.md**: 変更なし。
  * **仕様書**: `specs/system_specification.md`「位置情報付きメディア表示機能」に拡大プレビューの挙動を追記。
  * **設計書**: `designs/technical_design.md`に新規節「写真の拡大プレビュー表示（Issue #108）」を追加。
...
* **修正の動機・概要**:
  ...
  - 同フックは`backfillStatus.isRunning`が`true`になったことを**観測**して初めて表示状態になる設計だった。対象件数が極端に少ない・レート制限間隔が極小のE2E環境（フィクスチャ3件・50ms間隔）では、開始操作から最初の状態取得までの間に処理が完了してしまい、`isRunning: true`を一度も観測できないことがある（`null`から直接`isRunning: false`（完了済み）へ遷移する）。この場合フッターが最後まで表示されず、完了テキストがどれだけ待っても見つからなかった。
  - 調査の過程で、システム全体のCPU負荷（同一マシン上で稼働する他の無関係なプロセス）が高いほど再現しやすい傾向が見られたためタイムアウトを20秒→45秒に延長して検証したが、それでも失敗が再現し、対症療法では解消しないことを確認した。その後、上記の根本原因を特定し修正、45秒への延長は撤回し元の20秒へ戻した。
* **各ファイルへの影響と変更内容**:
  * **実装**:
    - `useBackfillProgressFooter.ts`: `show()`を新規公開。表示トリガーを`isRunning: true`の観測から、呼び出し元が開始操作と同期的に呼ぶ`show()`へ変更（`isRunning: true`を観測した場合に表示する既存のロジックは、そのまま安全策として残す）。
    - `MapWorkspace.tsx`: 初期取り込み・強制再取得の開始ボタンのクリックハンドラで、`startBackfill`/`startForceRefetch`呼び出しと同期的に`showBackfillFooter()`を呼ぶよう変更。
    - `electron/tests/bicycle-log.spec.ts`・`playwright.config.ts`: 調査中に一時的に延長したタイムアウト（45秒/60秒）を、根本原因の修正確認後に元の値（20秒/30秒）へ戻した。
  * **README.md**: 変更なし。
  * **仕様書**: 変更なし（内部実装のみで、ユーザーから見た挙動（進捗フッターの表示）に変化は無い。むしろ意図した挙動により忠実になった）。
  * **設計書**: `designs/technical_design.md`の「自転車ログバックフィル機能」に、進捗フッターの表示トリガーの設計とIssue #86で判明した競合条件を追記。`test_rules.md`にも本問題の調査過程・根本原因・対症療法（タイムアウト延長）では解消しなかった経緯を記録。
...
* **実データ実行時に発見・対応した追加の問題（同PRへコミット追加）**:
  - 残り37年月を実データで処理中、約9.3GiBの単一アーカイブ（2020-09、part列導入以前の既存アーカイブ）の処理でyauzlが「End of central directory record signature not found」で失敗した。調査の結果、`downloadFileToPath`のダウンロード自体はContent-Lengthと完全に一致しておりダウンロード起因の破損ではなく、標準の`unzip`コマンドでも同じエラーが再現したことから、**Drive上に保存されている元のzipファイル自体が（おそらく本処理より前の初期バックフィル時から）central directoryの途中で欠落した状態で破損していた**ことが判明した。
  - この診断過程で、ダウンロードが途中で打ち切られた場合に検出できていなかった潜在的な不具合も見つかったため、`downloadFileToPath`にContent-Lengthとの一致検証を追加し、ストリーミングダウンロード専用のタイムアウトを5分から120分へ延長した（今回の2020-09の破損はこの経路の問題ではなかったが、他の月で将来同種の問題が起きた場合に検出できるようにするための予防的対応）。
  - 1つの年月の処理失敗（上記のような復旧不能なアーカイブ破損を含む）でバッチ全体が止まらないよう、年月ごとの処理を`try/catch`で囲み、失敗した年月をログに記録した上で次の年月へ処理を継続するようオーケストレーションを変更した。失敗した年月は`video_stripped_year_months`に記録されないため、次回実行時も引き続き対象になる。
  - 2020-09自体の復旧（該当写真・動画データが実際に失われているか、別の方法で読み出せるか）はデータ整合性に関わる判断のため自動対応せず、ユーザーへ別途報告する。
* **実データ処理の最終結果（全178年月完了）**:
  - 残り37年月（旧2GiB安全弁でスキップされていた分）のうち、28年月をストリーミング処理で正常に完了（動画1,279件削除）。
  - 残り9年月（2020-09, 2024-02, 2024-03, 2024-08, 2024-11, 2025-03, 2025-04, 2025-05, 2025-11）は、いずれも「part分割前のレガシー単一アーカイブ（part=-1）かつ合計サイズが約4GiB以上」という条件で確定的に失敗した。標準ZIP形式は32bitオフセットで4GiBが上限のため、これらのファイルを書き込んだ当時（part分割導入以前の初期バックフィル）の`adm-zip`がZIP64拡張に対応しておらず、書き込み時点で既に壊れた状態だったと推定される（実測境界: 失敗した最小は約4.21GiB、成功した単一アーカイブの最大は約3.97GiB）。標準の`unzip`コマンドでも読めないことを確認済みで、ダウンロードや本処理自体に起因する破損ではない。
  - Issue #97・#99を通じた最終結果: `photos`テーブル49,879件→47,441件（動画2,438件削除）、`monthly_photo_archives`296行→178行（全年月が単一アーカイブに統合。9年月の失敗分も元々part分割されていない単一アーカイブだったため行数への影響はない）。
  - この9年月の復旧要否（該当写真・動画が実際に失われているか、元のGoogle Photosデータから再取り込みするか等）はデータ整合性に関わる判断のため自動対応せず、ユーザーの確認を待つ。
...
* **修正の動機・概要**:
  ...
  - 調査の結果、`usePhotos`の`isLoading`はメタデータ取得（`GET /activities/:id/photos`、撮影日時等のみを返す軽量なDBクエリ）の完了のみを表しており、写真の実バイナリ取得（`GET /photos/:id/image`、写真ごとに個別・非同期でブラウザが発行する`<img>`の読み込みであり、Reactの状態管理外）の完了を全く反映していなかったことが原因と判明した。バックエンド側は1つのアクティビティの写真が同じ月別アーカイブzipに集中することを前提に、写真1件分の表示のために月全体のzip（最大1GiB程度になりうる、Issue #23参照）をGoogle Driveから丸ごとダウンロードすることがあり、これが「表示までに数十秒」の主因だった。
  - ユーザーと相談の上、「全件読み込み完了を待って一斉表示」ではなく「メタデータ取得後は即座に全グリッドを表示し、各グリッドは自身の画像読み込みが完了するまでファイル名＋ローディングアイコンを表示し、読み込めたものから順に表示する」という方針で対応した。
* **各ファイルへの影響と変更内容**:
  * **実装**:
    - `frontend/src/components/ActivityDetailSidebar.tsx`: `PhotoGridItem`（新規）を追加。写真1件ごとに`isImageLoaded`（`useState`）で読み込み完了を個別管理し、未完了の間は`Image`を`visibility="hidden"`にしつつファイル名＋`Spinner`を重ねて表示、`onLoad`/`onError`で読み込み済みへ切り替える。`PhotoGrid`は`PhotoGridItem`を写真の枚数分レンダリングするだけに変更。
    - 対応する単体テストを追加（TDD、Red-Green）。単体テスト（フロントエンド全34ファイル307件）・lint・typecheck・型キャストチェックは全てGreen。
  * **README.md**: 変更なし。
  * **仕様書**: `specs/system_specification.md`の「位置情報付きメディア表示機能」章に、「写真を取得中...」がメタデータ取得のみを表す旨・写真ごとの個別ローディング表示について追記。
  * **設計書**: `designs/technical_design.md`の該当箇所に、原因（メタデータ取得と実バイナリ取得の完了が別物であること）と対応内容（`PhotoGridItem`の設計）を追記。
