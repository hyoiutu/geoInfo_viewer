...
- **テストファイル間の並列実行を許可しない**: 全テストファイルがE2E用DB・モックStravaサーバーという単一の共有プロセス/状態を操作するため、`playwright.config.ts`で`workers: 1`を設定し必ず直列実行する。並列実行すると、あるファイルの`beforeAll`が別ファイルの実行中データを`__test__/reset`等で消してしまう。
