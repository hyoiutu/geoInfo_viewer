...
- **共通コンポーネントが持つ`aria-label`が別の無関係な要素と重複していると、前のダイアログが閉じきる前のタイミング次第で`getByRole`が断続的に複数ヒットする**: `AppDialog`の閉じる(×)ボタンは共通の`aria-label="閉じる"`を持つが、`BackfillProgressFooter`の閉じるボタンも同じ`aria-label="閉じる"`を使っていたため、直前に開いていた`SettingsDialog`（ボタン押下と同時に`onClose()`を呼び自身を閉じる設計）がDOMから消えきる前に`getByRole('button', { name: '閉じる' })`を呼ぶと、`TestingLibraryElementError: Found multiple elements`で断続的に失敗した（フルスイート実行時の方が発生頻度が上がるが、実行順序・CPU負荷に関わらず起こり得る競合。PR #111レビュー対応、Issue #86）。**前のダイアログを閉じる操作を伴うテストでは、その要素（例: ダイアログ内の固有ボタン）が実際にDOMから消えたことを`await waitFor(() => expect(queryByRole(...)).not.toBeInTheDocument())`で先に待ってから、共通ラベルを持つ別要素を`getByRole`すること**（`toggleLayerViaDialog`ヘルパーが既にこのパターンを採用している）。
- **逆に、`ChakraProvider`配下外での使用を前提に設計されたコンポーネントは`renderWithChakra`を使わない**: `maplibregl.Marker`用に独立したReact rootへマウントされるコンポーネント（`PhotoBalloonThumbnail`・`PhotoBalloonClusterBadge`等、`react_rules.md`の「サードパーティライブラリがDOM要素を要求する場合」パターンで作られるもの）は、実装自体がChakra UIコンポーネントを使わずプレーンなDOM要素として書かれている。これらのテストで`renderWithChakra`を使うと、実際の実行時環境（`ChakraProvider`の外）と異なる環境でテストすることになり、将来コンポーネントが誤って`useToken`等のChakraコンテキストに依存する変更をされても、テストでは`ChakraProvider`が存在するため検出できず、実際のマーカー描画時にのみ失敗が顕在化するリスクがある。素の`render`（`@testing-library/react`）を使い、実際のマウント環境を再現すること（PR #117レビュー対応）。
- **Chakraの`Switch`・`Checkbox`（どちらもArk UI/`@zag-js`ベース）のクリックテスト**: `Switch.HiddenInput`/`Checkbox.HiddenInput`への`fireEvent.click`はDOM上の`checked`をネイティブ挙動として同期的に反転させるが、`onCheckedChange`コールバックの呼び出しは内部の状態管理（`@zag-js/react`の`bindable`）を経由するため次のtickにずれ込む。`fireEvent.click`直後に同期的に`expect(onCheckedChange).toHaveBeenCalledWith(...)`を書くと、実装が正しくてもテストが失敗する（`checked`propに実際の状態更新を返さないモック関数だけを渡した場合に特に顕在化しやすい）。`await waitFor(() => expect(...))`で待つこと。
...
- **`mockImplementation`を意図的に「解決しないPromise」等へ差し替えた場合、復元は`afterEach`で行う（テスト本文末尾に書かない）**: `beforeEach`の`vi.clearAllMocks()`は呼び出し履歴のみをクリアし`mockImplementation`は戻さないため（前項参照）、手動での復元（`mockResolvedValue(...)`への戻し）が必要になる。この復元をテスト本文の最後の行に書くと、それより前のアサーションが1つでも失敗した場合、例外がスローされた時点で以降の行（復元処理を含む）は実行されず、差し替えたモックの状態が後続の無関係なテストへ漏れ、原因の分かりにくい二次的な失敗（タイムアウト等）を引き起こす。テストの成否によらず必ず実行される`afterEach`で復元すること（PR #110レビュー対応）。
- **「一定時間待っても状態が変化しないこと」を確認する場合、`waitFor(() => expect(状態))`を素朴に使うと検証が成立しないことがある**: `waitFor`はコールバックを即座に（同期的に）1回目を試し、その時点で成功すればすぐ解決する。状態変化が非同期（Promiseのマイクロタスク・React再レンダー等）を経て後から起きる場合、`await expect(waitFor(() => expect(要素が存在する)), {timeout}).resolves.toBeTruthy()`は「変化が起きる前の初期状態」を捉えて即座に成功してしまい、本来検知したいはずの「後から変化してしまう」不具合を見逃す（実際にミューテーションテストで確認: 状態変化ロジックを意図的に壊しても、このパターンのテストは成功したままだった）。「まだ変化していないこと」を正しく検証するには、否定条件を`waitFor`し失敗することを期待する`await expect(waitFor(() => expect(要素が存在しないこと)), {timeout}).rejects.toThrow()`のパターンを使うこと（要素が存在する間は否定条件のチェックが失敗し続けリトライされるため、タイムアウトまでの間ずっと「まだ存在する」ことを継続的に検証できる）。新しく書いた「〜のままである」系のテストは、検証対象のロジックを一時的に壊した状態でも実際に失敗する（Red確認）ことをコミット前に確認すると安全（PR #110レビュー対応）。
- **`vi.clearAllMocks()` は呼び出し履歴のみをクリアし、`mockReturnValue`/`mockImplementation`で上書きした戻り値・実装は残る**: 上記の`vi.clearAllMocks()`は「モックファクトリで作成した関数のクリーンアップ」の文脈で`vi.resetAllMocks()`と並列に書いているが、両者は同一ではない。`mock.calls`/`mock.results`等の呼び出し履歴はどちらもクリアするが、個々のテストが`mapInstance.someMethod.mockReturnValue(...)`のように戻り値を上書きした場合、その上書きは`vi.clearAllMocks()`では取り消されず後続のテストへ持ち越されてしまう（`vi.resetAllMocks()`は実装自体を空の関数へ戻すため上書きも消える）。`beforeEach(() => vi.clearAllMocks())`だけを使っているファイルで、モックファクトリ内の同一インスタンス（`vi.mock('maplibre-gl', ...)`のMapモック等、テストファイル全体で使い回される`vi.fn()`）に対して一部のテストだけが`mockReturnValue`で戻り値を上書きしている場合、実行順序によっては上書きが後続テストに漏れて意図しない挙動になることがある（実際に発生した事故: `MapView.tests.tsx`で`queryRenderedFeatures`の戻り値を上書きしたテストの後に実行される別のテストが、上書きされた値をデフォルト値だと誤認してテストが通ってしまっていた。PR #113レビュー対応で発覚し、`afterEach`で当該モックを明示的に既定値へリセットして解消した）。個々のテストにその場しのぎの後始末を書くのではなく、`afterEach`（または次のテストの`beforeEach`）で、上書きされうるモックの戻り値を毎回既定値へ戻す仕組みを用意すること。
...
- **スタンドアロンCLIスクリプト（`seed-municipalities.ts`等、NestJSのDIコンテナを経由せず`ts-node`で直接実行する運用のスクリプト）は、単体テストがオーケストレーション部分（`process.argv`からの引数取り出し・CLIとしてのエントリポイント）をカバーしない**: このプロジェクトの既存方針でこれらのスクリプトは専用の単体テストファイルを持たず、テストを書く場合もエクスポートされた個々の関数を直接呼び出す形になるため、`process.argv[2]`のような引数解析コード自体はテストを通らない。そのため、リリース前に実際の呼び出しコマンド（`pnpm --filter backend run <script> -- <args>`等、想定される引数の渡し方を含む）で最低1回、実際にCLIとして実行して動作確認すること。
  - **実例**: `flatten-local-photo-directory.ts`（Issue #23）で、`pnpm --filter <package> run <script> -- <args>`がnpm scriptsの一般的な挙動と異なり区切りの`--`自体を除去せずそのまま`process.argv`へ渡すため、位置引数が1つずれて`ENOENT`になる不具合があった。単体テスト（エクスポート関数を直接呼ぶ形）・lint・typecheckは全てGreenだったが、実際にCLIとして実行するまで気づけなかった（2026-07-21、ユーザーの実行により発覚）。
  - **実例2**: `strip-videos-and-generate-thumbnails-locally.ts`（Issue #104、PR #114）で、複数の出力先パス（`thumbnailDirectoryPath`・`deletionLogPath`）のうち一部（`thumbnailDirectoryPath`）だけ`mkdirSync(path, { recursive: true })`で親ディレクトリを事前作成し、もう一方（`deletionLogPath`）を漏らしていた。`appendFileSync`はファイル自体は無ければ作成するが親ディレクトリまでは作成しないため、`deletionLogPath`の親ディレクトリが存在しない場合に動画を1件検出した時点でtry/catch外の`ENOENT`例外が発生しスクリプト全体が停止する不具合だったが、単体テスト・lint・typecheckはいずれもGreenのまま気づけなかった（pr-review-rallyスキルのレビューラリーで指摘され発覚、2026-07-30）。**複数の出力先パスを引数に取るCLIスクリプトでは、そのうち1つだけ動作確認して満足せず、全ての出力先パス（特にネストしたディレクトリを指定されうるもの）について「親ディレクトリが存在しない状態」を実際にCLI実行して確認すること。**
- **ディスク上のzipをエントリ単位で逐次読み書きするストリーミング処理（`zip-streaming.util.ts`利用箇所、`generate-thumbnail-archive-streaming.util.ts`等）のファイル名衝突テストは、衝突相手のエントリを変換対象より後ろへ配置したフィクスチャで書く**: 全エントリをメモリ上に保持してから処理する実装（例: AdmZipでzip全体を読み込む版）は元のzip内でのエントリの並び順に依存せず正しく衝突を検出できるが、1パスでストリーミング処理する実装は、変換対象エントリより後ろにある衝突相手のエントリ名をまだ知らないまま新しい名前を決めてしまい、出力zipに同名エントリが重複する不具合を生みうる。この不具合はエントリの並び順によって再現有無が変わるため、衝突相手を変換対象より前に置いたフィクスチャだけでテストすると、実装が「全エントリ名を事前収集してから処理する」設計になっていなくても見逃してしまう。衝突相手を意図的に後ろへ置いたテストケースを用意し、この順序依存を実装の変更（1パス化等）で壊さないことを保証すること（`convert-heic-archive-entries-streaming.util.tests.ts`、PR #116レビュー対応）。
...
- **Chakra UI `Switch`・`Checkbox`（レイヤートグル等）のクリック**: `getByRole('checkbox', { name: ... }).click()`（`force`の有無を問わず）は**動かないことがある**。`Switch`では隠しinputへのforceクリックでReactの制御状態`checked`が切り替わらないことがあり、後続の状態変化に依存するアサーションが永久にタイムアウトする。`Checkbox`では、視覚的に隠されたinput要素の実座標（`clip: rect(0px)`等で1px四方に縮小されている）が、ダイアログ本体等の他要素と画面上で重なり「`<div class="chakra-dialog__body ...">intercepts pointer events`」というエラーでクリック自体が失敗することがある（レイヤー切り替えダイアログ内のチェックボックスで実際に発生、Issue #32）。いずれもPlaywrightの自動操作特有の問題であり、実際のユーザー操作では発生しない（アプリ側のバグではない）。実ユーザーは`<label>`内の可視部分（スイッチ・チェックボックスの見た目やラベルテキスト）をクリックし、ブラウザ標準のlabel→input転送で隠しinputまで正しくトグルされる。`Switch.Root`・`Checkbox.Root`はどちらも隠しinputと`for`で紐づいた`<label>`としてレンダリングされるため、代わりに`getByText('<ラベル文字列>', { exact: true }).click()`でラベルのテキスト（実ユーザーがクリックするのと同じ可視部分）をクリックすること。
...
- **`bicycle-log.spec.ts`の初期取り込み完了待ちがタイムアウトすることがある（Issue #86、根本原因判明・修正済み）**: `getByText('取得が完了しました')`の待機（`BACKFILL_COMPLETE_TIMEOUT_MS`）が断続的にタイムアウトする問題を複数回調査した末、`useBackfillProgressFooter`のバグと判明した。同フックは`backfillStatus.isRunning`が`true`になったことを**観測**して初めて進捗フッターを表示する設計だったが、対象件数が極端に少ない・レート制限間隔が極小のE2E環境では、開始操作（`startBackfill`）から最初の状態取得（`refresh`）までの間に処理が完了してしまい、`isRunning: true`の状態を一度もフロントエンドが観測できないことがある（開始操作の直後、`backfillStatus`が`null`から直接`isRunning: false`（完了済み）へ遷移する）。この場合フッター自体が最後まで表示されず、`getByText('取得が完了しました')`がどれだけ待っても見つからない。修正は、フッターの表示トリガーを「`isRunning: true`の観測」から「開始操作そのもの（`useBackfillProgressFooter`が新たに公開する`show()`を開始ボタンのクリックハンドラで同期的に呼ぶ）」へ変更し、ポーリングの結果を待たずに確実に表示されるようにした。`git worktree`での旧コミット比較で実装差分が原因でないと判断していた過去の調査は誤りではなく、このバグは調査対象のコミットでも既に存在していた（本問題自体は昔からあったが、タイミング条件が揃わないと顕在化しない性質のため見つかっていなかった）。なお、調査中にシステム全体のCPU負荷（同一マシン上で稼働する他の無関係なプロセス）が高いほど再現しやすい傾向も観測されたが、これは上記の競合が発生する確率を上げる一因ではあっても、タイムアウトを伸ばすだけでは解消しない（実際に45秒まで延長しても失敗が再現した）根本原因ではなかった。
