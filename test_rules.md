...
> **TODO**: 本セクションは frontend/backend の雛形構築（別タスク）が完了した時点で、確定した設定に置き換えること。現時点で踏襲予定・要検討の事項を以下に記す。

- **テストフレームワーク**: Vitest + Testing Library（`@testing-library/react` / `@testing-library/dom` / `@testing-library/jest-dom`）を frontend/backend 双方で使用する想定。
- **Chakra UIを使用するコンポーネントのテスト**: ChakraのコンポーネントはSystemコンテキスト（`ChakraProvider`）を要求するため、`@testing-library/react`の`render`をそのまま使わず、`ChakraProvider`でラップ済みのカスタムrenderヘルパー（例: `frontend/src/test-utils/renderWithChakra.tsx`）を用意して使うこと。また、Chakraの`Dialog`（Ark UI）はクローズ処理やbackdropの外側クリック検知を内部で非同期に処理するため、状態変化を伴うアサーションは`waitFor`で待つ必要がある場合がある。backdropクリックのように内部実装が「外側クリック検知」に依存する挙動をアサーションしたい場合は、コンポーネント側で明示的な`onClick`ハンドラを持たせておくとテストが安定する。
- **共有モック・フィクスチャ**: 複数のテストファイルで共有するモック（例: Electronの`window.api`のスタブ、地図データのダミーフィクスチャ）は各ワークスペースの `test-utils/` ディレクトリに配置する。このディレクトリのファイルはテスト対象そのものではないため `*.tests.*` という命名は使わない（Vitestの実行対象に含めないため）。
...
- **バックエンド（NestJS + PostgreSQL/PostGIS）のテスト方針**: 未検討。ORM選定、テスト用DB（例: testcontainers、専用テストスキーマ等）の用意方法、PostGISを含む空間クエリのテスト方法は、バックエンド雛形構築時にあわせて本セクションに追記すること。
- **実行コマンド**: 雛形構築後に確定するコマンドを commit_rules.md の「開発プロジェクト固有の検証手順」とあわせて記載する。
