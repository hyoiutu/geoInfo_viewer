...
本ルールは`pnpm run check:absolute-paths`（`scripts/check-absolute-paths.mjs`）で機械的に検出できる（PR #55レビュー対応で追加）。現時点ではコミット時の自動実行には組み込んでおらず、手動実行のみ。
...
本ルールのうち「モジュールトップレベルの関数・クラスメソッドにTSDocが1件も無い」ことは`pnpm run check:tsdoc`（`scripts/check-tsdoc.mjs`）で機械的に検出できる（PR #55レビュー対応で追加）。ただし上記の詳細な書き方（オブジェクト型の抽出、ネストした関数の扱い等）まではチェックしておらず、TSDocの有無のみを検出する点に注意。現時点ではコミット時の自動実行には組み込んでおらず、手動実行のみ。
...
本ルールは`pnpm run check:useeffect-comments`（`scripts/check-useeffect-comments.mjs`）で機械的に検出できる（PR #55レビュー対応で追加）。現時点ではコミット時の自動実行には組み込んでおらず、手動実行のみ。
