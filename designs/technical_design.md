...
- `convertHeicArchiveEntriesStreaming`（新規、`convert-heic-archive-entries-streaming.util.ts`）は、ディスク上の月別アーカイブzip（`sourceZipPath`）と対象エントリパス一覧を受け取り、実際にHEICであるエントリのみ`convertHeicBufferToJpegBuffer`でJPEGへ変換し、`.jpg`拡張子のエントリへ置き換えた新しいzipを`destZipPath`へストリーミング（エントリ単位の逐次読み書き）で生成する（衝突時は`resolveUniquePath`で連番、新規エントリはSTORED）。拡張子だけHEICで中身が実際には別形式のエントリ・変換対象外のエントリは変換せずそのまま出力へ引き継ぐ（サムネイル生成と同じ方針）。1件の変換失敗は他のエントリの変換を止めず、失敗したエントリは元のHEICエントリのまま出力へ残る。
  - **PR #116レビュー対応でBuffer全体をメモリへ載せる方式（`convertHeicArchiveEntries`、AdmZipでのランダムアクセス編集）からストリーミング方式へ変更した**: `generate-thumbnail-archive-streaming.util.ts`・`consolidate-monthly-archive-streaming.util.ts`と同じ`zip-streaming.util.ts`（`forEachZipEntry`/`openEntryReadStream`/`readStreamToBuffer`/`writeYazlOutput`）を使い、1エントリ分（写真1件、数MB程度）のみを一時的にBufferとして保持する。既存アーカイブは月合計サイズが数GB〜十数GBになりうり、Buffer全体をメモリへ載せる方式では`generate-thumbnail-archives.ts`がストリーミング方式へ切り替える契機となったOOM事故（Issue #99、16GB機で16.6GB単一zipの処理に失敗）を、複数アーカイブを1プロセス内でループ処理する本パイプラインでも再発させるリスクがあったため。
  - 変換前に、変換対象を処理する前段として`sourceZipPath`の全エントリ名を1度収集してから本処理を行う2パス構成にしている。1パスでストリーミング処理すると、変換対象エントリ（例: `IMG_1.heic`）より後ろのエントリ（例: 既存の`IMG_1.jpg`）とのファイル名衝突を、そのエントリへ到達するまで検出できず、出力zip内に同名エントリが重複してしまう恐れがあるため。
- `convert-heic-photos-to-jpeg.ts`（新規、`pnpm --filter backend run convert-heic:photos`）がオーケストレーションを担う。`generate-thumbnail-archives.ts`と同じ「未処理分を検出して処理する恒久パイプライン」のパターンを採用しており、専用のone-offスクリプトは用意していない（Issueの要求通り、初回実行で新規取り込み分・既存アップロード済みのバックログの両方を自動的にカバーする）
  ...
  - `photos.source_file_id`（月別アーカイブzip）単位でグループ化し、1つのzipにつき1回のダウンロード・アップロードで対象写真をまとめて変換する。ダウンロード・アップロードは`downloadFileToPath`/`uploadFileFromPath`（ディスク経由のストリーミング）を使い、`generate-thumbnail-archives.ts`と同じくアーカイブ全体をメモリへ保持しない。変換後のzipは同じ`driveFileId`へ上書きする（動画削除・統合の`strip-videos-and-consolidate-archives.ts`とは異なり、新規ファイル作成＋旧ファイル削除の手順は取らない。複数の元ファイルを統合するわけではなく1つのアーカイブを編集するだけのため、`MonthlyPhotoArchiveService.reorganize`の追記時と同じ「既存ファイルへの上書き」で十分と判断した）
  ...
  - **Drive更新後のDB更新失敗によるデータ不整合対策（PR #116レビュー対応）**: Drive側のzip上書き（`uploadFileFromPath`）が成功した後に`photoRepository.save`が失敗すると、Drive上ではエントリ名が既に`.jpg`へ変わっているのにDBは旧`.heic`パスを保持したままになる。この状態で再実行すると、`heicPhotos`検出クエリは当該写真を対象に含め続けるが、`convertHeicArchiveEntriesStreaming`は（Driveの実エントリが既にリネームされているため）対象パスをzip内に見つけられず「対象パスが存在しない」正常系として静かにスキップし続け、恒久的に検出不能な不整合になる。この`save`失敗は専用のtry/catchで捕捉し、Drive側は更新済みだがDBが追随できていない旨と、手動でのDB修正に使える変換前後のパス対応表をログへ明示してから再送出する（アーカイブ単位の失敗として`failedArchiveIds`にも記録される）。DB側を先に更新する順序へ入れ替えても、今度はDriveのアップロードが失敗した場合に同種の逆向きの不整合（かつ次回実行時の検出クエリから外れてしまいさらに気づきにくい）が生じるため、順序の入れ替えでは解決しない。ログによる可視化・手動修正を前提とした対策とした
