...
### Google Takeoutの写真データのフラット化（任意）

Google Takeoutで一括ダウンロードした写真をローカルへ展開すると、アルバム単位等でディレクトリが細かくネストされた状態になります。以下を実行すると、ネストされたディレクトリ以下の全ファイル（写真・JSONサイドカー）をサブディレクトリの無い1つのフラットなディレクトリへコピーできます（既存写真の一括取り込みの入力形式に合わせるためのものです。取り込み手順は次項参照）。

```bash
pnpm --filter backend run flatten:photos-local -- <展開済みディレクトリの絶対パス> <出力先ディレクトリの絶対パス>
```

1枚の写真が複数のアルバムに属する場合、同一内容のファイルが複数のディレクトリに重複して含まれることがありますが、内容が完全に一致するファイルは自動的に1件へ集約されます（内容が異なる同名ファイルは連番を付けて別ファイルとして保存されます）。元のディレクトリ構造・ファイルは変更されません（コピーのみ）。
...
1. 手元でTakeoutのzipをすべて展開し、写真本体とJSONサイドカーファイル（`*.json`）をサブディレクトリを作らず1つのフラットなローカルディレクトリへ集約する（ディレクトリがネストしている場合は前項のフラット化ツールを使う）
...
写真本体の実バイナリは、処理単位（撮影年月ごとに、さらにサイズ上限ごとの複数partへ分割したもの）でのみメモリへ読み込むため、対象件数が多くてもメモリ使用量は増え続けません。また、対象件数が多く実行に長時間かかる場合に途中で中断しても、最後まで処理済みの単位は次回実行時に自動でスキップされるため、同じディレクトリに対して再実行して問題ありません（処理の途中で中断された単位は自動スキップされず再処理されます）。

Node.jsの制約上2GiBを超えるファイル（動画等）は読み込めないため、対象ディレクトリに含まれていても自動的にスキップされます（実行完了時にスキップしたファイルのパス一覧が出力されるので、必要であれば個別に対応してください）。
