...
Google Takeoutで一括ダウンロードした写真をローカルへ展開すると、アルバム単位等でディレクトリが細かくネストされた状態になります。以下を実行すると、ネストされたディレクトリ以下の全ファイル（写真・JSONサイドカー）をサブディレクトリの無い1つのフラットなディレクトリへコピーできます（既存写真の一括取り込みの入力形式に合わせるためのものです。取り込み手順自体は別途用意されます）。
