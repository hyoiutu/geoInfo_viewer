...
アクティビティ詳細画面に通過自治体を表示する機能は、あらかじめ市区町村境界データをDBへ投入しておく必要があります。マイグレーション適用後、以下を実行してください（全国47都道府県分のデータをダウンロードするため数分かかります）。

```bash
pnpm --filter backend run seed:municipalities
```

データは[政府統計の総合窓口(e-Stat)地図で見る統計(統計GIS)提供の市区町村界データ（GeoShapeリポジトリ、2023-01-01時点、高解像度版）](https://geoshape.ex.nii.ac.jp/city/choropleth/)を使用しています。再実行すると毎回全件洗い替えします。
