...
データは[政府統計の総合窓口(e-Stat)地図で見る統計(統計GIS)提供の市区町村界データ（GeoShapeリポジトリ、高解像度版）](https://geoshape.ex.nii.ac.jp/city/choropleth/)を使用しています。現行（最新）データに加え、2000-10-01（平成の大合併前）時点のデータも投入します（`backend/src/municipalities/era.constants.ts`の`MUNICIPALITY_ERAS`で年代を追加可能）。再実行すると年代ごとに全件洗い替えします（他の年代のデータには影響しません）。
