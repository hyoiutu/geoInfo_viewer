...
写真本体の実バイナリは、撮影年月ごとの処理単位でのみメモリへ読み込むため、対象件数が多くてもメモリ使用量は増え続けません。また、対象件数が多く実行に長時間かかる場合に途中で中断しても、最後まで処理済みの年月は次回実行時に自動でスキップされるため、同じディレクトリに対して再実行して問題ありません（処理の途中で中断された年月は自動スキップされず再処理されます）。
